...
const MOTIFS = [
  { value: 'faux_pro',            label: 'Fausse identité professionnelle' },
...
export default function SignalerButton({ reportedUserId, isLoggedIn }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/signalements', {
      ...
      body: JSON.stringify({ reported_user_id: reportedUserId, motif, message }),
    })
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 text-xs text-navy/30 hover:text-red-500 transition-colors px-3 py-2 rounded-lg hover:bg-red-50"
      >
            ...
            {done ? (
              <div className="text-center py-4">
                ...
                <p className="text-sm text-navy/50 mb-5">
                  Merci. Notre équipe examinera ce profil et prendra les mesures nécessaires.
                </p>
              ...
              <>
                <div className="flex items-start justify-between mb-5">
                  <div>
                    <h3 className="font-serif text-xl text-navy">Signaler ce profil</h3>
                    <p className="text-xs text-navy/45 mt-1">Votre signalement sera examiné par notre équipe.</p>
                  </div>
                  <button onClick={handleClose} className="text-navy/30 hover:text-navy/60 transition-colors text-xl leading-none mt-0.5">
                    ×
                  </button>
                </div>
                ...
                {!isLoggedIn ? (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                    Vous devez être <a href="/auth/login" className="font-semibold underline">connecté</a> pour signaler un profil.
                  </div>
                ) : (
