...
export default function VisiteCalendar({ visites }: Props) {
  ...
  return (
    <div>
      {/* Navigation mois */}
      <div className="bg-white rounded-2xl border border-navy/08 mb-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-navy/06">
          <button onClick={prevMonth}
            className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-navy/06 transition-colors text-navy/50 hover:text-navy">
          ...
          <div className="flex items-center gap-3">
            <span className="font-serif text-xl text-navy">{MOIS[month]} {year}</span>
        ...
        {/* En-têtes jours */}
        <div className="grid grid-cols-7 border-b border-navy/05">
          {JOURS.map(j => (
            <div key={j} className="py-2.5 text-center text-[11px] font-semibold text-navy/35 uppercase tracking-wider">
              {j}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          {grid.map((day, i) => {
            if (!day) return <div key={`null-${i}`} className="min-h-[80px] border-b border-r border-navy/04 bg-navy/01" />
            ...
            return (
              <button
                ...
                className={`min-h-[80px] p-2 text-left transition-all relative flex flex-col
                  ...
                  border-navy/05
                  ${isSelected  ? 'bg-primary/06 ring-1 ring-inset ring-primary/20' : 'hover:bg-navy/02'}
                  ${!isCurrentMonth ? 'opacity-40' : ''}
                `}
              >
                {/* Numéro du jour */}
                <span className={`text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full mb-1 flex-shrink-0
                  ${isToday ? 'bg-primary text-white' : 'text-navy/70'}
                `}>
                  {day.getDate()}
                </span>
                ...
                <div className="flex flex-col gap-0.5 w-full">
                  ...
                  {dayVisites.length > 3 && (
                    <div className="text-[10px] text-navy/40 px-1">
      ...
      <div className="flex items-center gap-5 mb-6 px-1">
        {Object.entries(STATUT_CONFIG).map(([key, cfg]) => (
          <div key={key} className="flex items-center gap-1.5 text-xs text-navy/50">
      ...
      {selectedDay && (
        <div className="bg-white rounded-2xl border border-primary/20 overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 bg-primary/04 border-b border-primary/10">
            <div>
              <div className="font-medium text-navy capitalize">{selectedDateLabel}</div>
              <div className="text-xs text-navy/45 mt-0.5">
                {selectedVisites.length} visite{selectedVisites.length > 1 ? 's' : ''} ce jour
              </div>
            </div>
            <button onClick={() => setSelectedDay(null)}
              className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-navy/08 text-navy/40 hover:text-navy transition-colors">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div key={v.id} className="px-5 py-4 flex items-start gap-4 flex-wrap">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {v.creneau && (
                        <span className="text-[11px] text-navy/45 bg-navy/05 px-2 py-0.5 rounded-full">
                          🕐 {v.creneau}
                        </span>
                      )}
                    </div>
                    ...
                    {v.biens && (
                      <a href={`/annonce/${v.biens.id}`}
                        className="text-xs text-primary hover:underline mb-1.5 block">
                        🏠 {v.biens.titre} — {v.biens.ville}
                      </a>
                    )}

                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-navy/50">
                      <a href={`mailto:${v.demandeur_email}`} className="hover:text-primary">
                        ✉️ {v.demandeur_email}
                      </a>
                      {v.demandeur_tel && (
                        <a href={`tel:${v.demandeur_tel}`} className="hover:text-primary">
                          📞 {v.demandeur_tel}
                        </a>
                      )}
                    </div>
                    ...
                    {v.message && (
                      <p className="mt-2 text-xs text-navy/55 bg-navy/03 rounded-lg px-3 py-2 italic">
                        « {v.message} »
                      </p>
                    )}
                  </div>
